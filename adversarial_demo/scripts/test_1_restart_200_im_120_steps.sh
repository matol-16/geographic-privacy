#!/usr/bin/env bash
#
# End-to-end test of every attack type on YFCC in a SINGLE evaluate-dataset run, with
...
N_IMAGES=200
RESULTS_DIR="${PROJECT_DIR}/results/cluster_eval/test_2206_1_restart_200_im_120_steps"
...
python main.py evaluate-dataset \
  ...
  --run-model-transfer-ablation \
  --eval-num-steps "${EVAL_NUM_STEPS[@]}" \
  --max-restarts "${MAX_RESTARTS}" \
  --override "attack_budgets.${DATASET}=${BUDGETS}" \
  --override "attack_train_args.${DATASET}.num_restarts=${RESTARTS}" \
  --override "attack_train_args.${DATASET}.yfcc.n_steps=${STEPS}"
